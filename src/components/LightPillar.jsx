...
const LightPillar = ({
  ...
  useEffect(() => {
    ...
    // High performance quality settings optimized for whole-page 60-120 FPS
    const settings = {
      iterations: 24,
      waveIterations: 2,
      pixelRatio: typeof window !== 'undefined' ? Math.min(window.devicePixelRatio || 1, 1.0) : 1,
      precision: 'mediump',
      stepMultiplier: 1.35
    };
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        col *= uGlowAmount;
        col = (exp(2.0 * col) - 1.0) / (exp(2.0 * col) + 1.0);

        if(uNoiseIntensity > 0.0) {
          float noise = fract(sin(dot(vUv, vec2(12.9898, 78.233))) * 43758.5453);
          col += (noise - 0.5) * uNoiseIntensity * 0.04;
        }
        ...
        alpha = smoothstep(0.0, 0.6, alpha) * 0.85;
    ...
    const uniforms = {
      ...
      uWaveSin: { value: Math.sin(waveSpeed) },
      uWaveCos: { value: Math.cos(waveSpeed) }
    };
    ...
    const animate = currentTime => {
      ...
      const rotAngle = timeRef.current * 0.5;
