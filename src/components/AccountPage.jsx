...
import { OrderTrackingProgress } from './TrackOrderForm';
...
function AccountDashboard({ orders, wishlist, addresses, unreadCount, activeOrder }) {
  return <><div className="account-section-heading"><div><p>// OVERVIEW</p><h2>ACCOUNT DASHBOARD</h2></div></div>
    ...
    <div className="account-quick-grid"><AccountQuickCard icon="fa-box" title="MY ORDERS" count={`${orders.length} ${orders.length === 1 ? 'order' : 'orders'}`} action="View orders" path="/account/orders"/><AccountQuickCard icon="fa-heart" title="WISHLIST" count={`${wishlist.length} saved ${wishlist.length === 1 ? 'item' : 'items'}`} action="View wishlist" path="/account/wishlist"/><AccountQuickCard icon="fa-location-dot" title="SAVED ADDRESSES" count={`${addresses.length} ${addresses.length === 1 ? 'address' : 'addresses'}`} action="Manage addresses" path="/account/addresses"/><AccountQuickCard icon="fa-bell" title="NOTIFICATIONS" count={`${unreadCount} unread`} action="View notifications" path="/account/notifications"/></div>
  </>;
}
